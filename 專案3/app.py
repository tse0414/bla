...
# 客戶用 /api/auth/register 動態加入（只在記憶體）
...
# ------------------------------------------------
# 建立客戶資料：/api/customers（寫到 Customers 工作表）
# 只有 staff / admin 可以用
# ------------------------------------------------
...
# ------------------------------------------------
# 建立包裹：/api/parcels
# 任何登入角色（customer / staff / admin）都可以建立
# ------------------------------------------------
@app.route("/api/parcels", methods=["POST"])
@token_required
def create_parcel():
    """
    給 parcel_tracking.html 使用：
    前端送來的欄位（payload）大致是：
      sender, receiver, receiverAddress, weight, volume, service_type
    這裡會：
      1) 產生追蹤編號 TRK-YYYYMMDD-XXXX
      2) 寫入 Excel 的 Parcels（9 欄）
      3) 回傳 tracking_no + package 給前端
    """
    data = request.get_json() or {}
